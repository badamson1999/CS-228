...
public class InfixExpression extends Expression 
{
	...
	public void postfix() throws ExpressionFormatException
	{
		...
		while (scan.hasNext()){
			...
			if(isInt(token)){
				...
				if (rank >1){
					throw new ExpressionFormatException("Operator Excepted");
				}
				continue;
			}
			...
			if (isOperator(c))
			{
				...
				if ((operatorStack.isEmpty()||operatorStack.peek().compareTo(op)<0)&&c!=')'){
				...
				if (c=='('){
					...
					rank--;
						...
						if (rank <0){
							throw new ExpressionFormatException("Operand Excepted");
						}	
		...
		while(!operatorStack.isEmpty()){
			...
			postfixString =postfixString +" "+top.getOp();
		}
	...
	public int evaluate() throws ExpressionFormatException, UnassignedVariableException  
    {
    	// TODO
		postfix();
		PostfixExpression e = new PostfixExpression(postfixString);
		
		e.setVarTable(varTable);
		return e.evaluate();  
    }
	...
	private void outputHigherOrEqual(Operator op)
	{
		...
		if (op.getOp()==')'&&operatorStack.peek().getOp()=='('&& !operatorStack.isEmpty()){
			operatorStack.pop();
		}
		if (op.getOp()!=')')
		operatorStack.push(op);
